...
end_et = spice_handler.utc_to_et("2020-02-05T11:00:00")  # 1 hour window
...
true_omega0 = np.deg2rad(np.array([0.5, -0.3, 2.0]))  # ~2°/s realistic tumbling
...
# Use adjacent epochs (0, 1, 2) — no brightness separation rule
ep1_idx = 0
ep2_idx = 1
ep3_idx = 2
...
if __name__ == '__main__':
    # Compute FFT omega bound from observed lightcurve
    ...
    omega_bound_deg = peak_freq * 360.0  # Hz → °/s (full rotation per cycle)
    log(f"FFT peak freq: {peak_freq:.6f} Hz, omega bound: {omega_bound_deg:.3f} °/s")
    log(f"True |ω|: {np.linalg.norm(np.rad2deg(true_omega0)):.3f} °/s")
    N_SEEDS = 10000
    ...
    if pairs_12 is None or pairs_23 is None:
        ...
        ep2_to_ep1 = defaultdict(list)
        ep2_to_ep3 = defaultdict(list)
        ...
        for i2 in common_ep2:
            ...
            ep1_list = ep2_to_ep1[i2]
            ep3_list = ep2_to_ep3[i2]
            
            if len(ep1_list) == 0 or len(ep3_list) == 0:
                continue
            
            # Compute all ω₁₂ vectors for this epoch-2 candidate
            omega_12_list = []
            for i1 in ep1_list:
                q1 = cands[0][i1]['quat']
                R1 = Rotation.from_quat([q1[1], q1[2], q1[3], q1[0]])
                dR = R2 * R1.inv()
                omega_12_list.append(np.rad2deg(dR.as_rotvec() / dt_12))
            omega_12_arr = np.array(omega_12_list)
            ...
            for i3 in ep3_list:
                ...
                dR = R3 * R2.inv()
                omega_23_list.append(np.rad2deg(dR.as_rotvec() / dt_23))
            omega_23_arr = np.array(omega_23_list)
            
            # KD-tree: find ω₂₃ vectors near each ω₁₂ vector
            tree = cKDTree(omega_23_arr)
            
            for idx_1, omega_12 in enumerate(omega_12_arr):
                matches = tree.query_ball_point(omega_12, r=OMEGA_THRESHOLD_DEG)
                
                for idx_3 in matches:
                    i1 = ep1_list[idx_1]
                    i3 = ep3_list[idx_3]
                    omega_23 = omega_23_arr[idx_3]
                    omega_diff = np.linalg.norm(omega_12 - omega_23)
                    
                    # Also check 1→3 bridge
                    q1 = cands[0][i1]['quat']
                    q3 = cands[2][i3]['quat']
                    R1 = Rotation.from_quat([q1[1], q1[2], q1[3], q1[0]])
                    R3 = Rotation.from_quat([q3[1], q3[2], q3[3], q3[0]])
            ...
            n_ep2_processed += 1
            if n_ep2_processed % 1000 == 0:
                log(f"    Processed {n_ep2_processed}/{len(common_ep2)} ep2 candidates, "
                    f"{len(surviving_triplets)} survivors so far ({time.time()-t_triplet:.0f}s)")
        
        dt_triplet = time.time() - t_triplet
        log(f"  Triplet matching time: {dt_triplet:.1f}s")
    ...
    outpath = PROJECT_ROOT / 'data/results/inversion_diagnostics/exp_omega_bridge_v3.json'
