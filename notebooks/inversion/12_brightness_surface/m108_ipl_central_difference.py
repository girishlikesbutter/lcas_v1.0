#!/usr/bin/env python3
"""
m108 — IPL Centroid Cost with Central Anchor.

Fixes m108's delta-q amplification failure by using a SHORT-WINDOW cluster
of K=4 tight IPL minima centered on the most central anchor. Grid+NM use only
the cluster (max |dt| ~6 min → PAB error < 5°). Geo extends to ALL tight minima.

Key insight: IPL centroids are 1-3° precision targets. Grid omega has ~0.67°
direction error which amplifies over time (PAB error ≈ 0.67° × ω × dt). By
keeping constraints within ~6 min of the anchor, we keep PAB error < 5°.

Population analysis: for K=4, 81/100 seeds have estimated grid cost < 0.1
(better than m102's BEST seeds at 0.05-0.20).

Usage:
  MICRO108_SEED=27 python3 m108_ipl_central.py
"""
...
TRAJ_SEED = int(os.environ.get('MICRO108_SEED', '27'))
...
N_PHI_COARSE = 36
N_PHI_FINE = 360
NM_TOP = int(os.environ.get('MICRO108_NM_TOP', '300'))
GEO_TOP = 20
LOFI_TOP = int(os.environ.get('MICRO108_LOFI_TOP', '300'))
PEAK_WINDOW = 3
...
IPL_K = 4                  # cluster size: anchor + 3 constraints
...
# ══════════════════════════════════════════════════════════════════════
CKPT_DIR = Path(os.environ.get('MICRO108_CKPT_DIR',
                str(RESULTS_DIR / f"m108_ipl_central" / f"seed_{TRAJ_SEED:03d}")))
CKPT_DIR.mkdir(parents=True, exist_ok=True)
_log_file = open(str(CKPT_DIR / "pipeline.log"), "w")
sys.stdout = Tee(sys.__stdout__, _log_file)

print("=" * 60, flush=True)
print(f"m108 — IPL Central Anchor (seed {TRAJ_SEED})")
print(f"  NM_TOP={NM_TOP}, GEO_TOP={GEO_TOP}, K={IPL_K}")
print(f"  IPL ang_dist_max={IPL_ANG_DIST_MAX}°")
print("=" * 60)
t_global = time.time()
...
all_tight = [m for m in minima_sorted
             if m['ang_dist_deg'] is not None and m['ang_dist_deg'] < IPL_ANG_DIST_MAX]

if len(all_tight) < IPL_K:
    print(f"WARNING: only {len(all_tight)} tight IPL minima (need {IPL_K})")
    print(f"  Relaxing threshold: using top minima by ang_dist regardless of threshold")
    all_tight = [m for m in minima_sorted if m['ang_dist_deg'] is not None][:max(IPL_K, 4)]

# ── Central anchor selection: minimize max(centroid_dist + PAB_error) ──
# PAB_error grows with |dt| from anchor due to grid omega imprecision.
# We want BOTH small centroid distances AND small |dt| — the combined
# score balances tightness of constraint vs delta-q amplification.
tight_eps = [int(m['ep']) for m in all_tight]
tight_times = [obs_times[ep] for ep in tight_eps]
tight_dists = [m['ang_dist_deg'] for m in all_tight]

# Estimate PAB error rate from grid resolution (~0.67° for 2000 Fibonacci dirs)
_omega_est_from_peaks, _ = find_peaks(-observed_lc, distance=5, prominence=0.3)
_omega_est_dps = 0.0397 * len(_omega_est_from_peaks) + 0.0417
_delta_omega = np.deg2rad(0.67) * np.deg2rad(_omega_est_dps)

best_anchor_i = 0
best_worst_combined = float('inf')
best_cluster = list(range(min(IPL_K, len(all_tight))))

for i in range(len(all_tight)):
    # For each candidate anchor, find K-1 members with lowest combined error
    members = []
    for j in range(len(all_tight)):
        if j == i:
            continue
        dt_sec = abs(tight_times[j] - tight_times[i])
        pab_err_deg = np.rad2deg(_delta_omega * dt_sec)
        combined = tight_dists[j] + pab_err_deg
        members.append((combined, j))
    if len(members) < IPL_K - 1:
        continue
    members.sort()
    cluster = [i] + [members[k][1] for k in range(IPL_K - 1)]
    # Score = worst combined error across all members (including anchor's own centroid dist)
    worst_combined = max(
        tight_dists[j] + np.rad2deg(_delta_omega * abs(tight_times[j] - tight_times[i]))
        for j in cluster if j != i
    )
    worst_combined = max(worst_combined, tight_dists[i])
    if worst_combined < best_worst_combined:
        best_worst_combined = worst_combined
        best_anchor_i = i
        best_cluster = cluster

# Reorder: anchor first, then constraints sorted by |dt| from anchor
cluster_minima = [all_tight[best_cluster[0]]]  # anchor
constraint_indices = sorted(best_cluster[1:],
                            key=lambda j: abs(tight_times[j] - tight_times[best_anchor_i]))
for j in constraint_indices:
    cluster_minima.append(all_tight[j])

# All tight minima (for geo refinement later)
all_tight_minima = all_tight

anchor_time_cluster = tight_times[best_anchor_i]
max_dt_cluster = max(abs(tight_times[j] - anchor_time_cluster) for j in best_cluster if j != best_anchor_i)
print(f"\nIPL census: {ipl_seed_data['n_minima']} total minima, {len(all_tight)} tight(<{IPL_ANG_DIST_MAX}°)")
print(f"Optimal cluster (K={IPL_K}, max |dt|={max_dt_cluster:.0f}s = {max_dt_cluster/60:.1f} min, "
      f"worst_combined={best_worst_combined:.1f}°):")
for i, m in enumerate(cluster_minima):
    ep = int(m['ep'])
    dt_sec = abs(obs_times[ep] - obs_times[int(cluster_minima[0]['ep'])])
    pab_err = np.rad2deg(_delta_omega * dt_sec)
    combined = m['ang_dist_deg'] + pab_err
    tag = " [ANCHOR]" if i == 0 else f" [dt={dt_sec:.0f}s, pab_err={pab_err:.1f}°, total={combined:.1f}°]"
    print(f"  #{i}: ep={m['ep']}, centroid_dist={m['ang_dist_deg']:.1f}°, "
          f"n_centroids={m['n_centroids']}, loops={m['loop_count']}{tag}")

# For grid/NM: use only the K-cluster
tight_minima = cluster_minima
...
# For geo cost, use ALL tight IPL minima (not just the K-cluster)
# At this point omega is refined to ~1-2°, so delta-q amplification is small
# and we can safely use distant constraints for extra discrimination
all_ipl_epochs = np.array([int(m['ep']) for m in all_tight_minima])
all_ipl_centroids = [np.array(m['centroid_dirs']) for m in all_tight_minima]
