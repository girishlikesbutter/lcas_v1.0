...
Hypothesis: a 600k super-Fibonacci SO(3) grid (~1.4 deg median geodesic
spacing, 10x denser than [[m127]]'s 60k ~3 deg grid) + top-50 L-BFGS-B
polish (omega fixed at -omega_true) recovers seed 33's known flipped-omega
basin (from [[m126]], hi-fi MSE ~0.082, q0 compensation 98.5 deg about
body +Z axis). Success = winner hi-fi < 0.15.
  CONFIRMED  iff control seed 33 hi-fi < 0.15 (pure grid-density is the fix
             for narrow flipped-omega basin recovery; [[m127]]'s REFUTED
             is retractable on wider grids).
  REFUTED    iff control seed 33 hi-fi >= 0.15 (narrow basins are NOT
             grid-density-recoverable at 600k; pivot to population-based
             (DE) enumeration in m130).

Positive control: seed 33. Its known flipped-omega basin width is
<0.001 deg per [[m126]] polish observations (dq0 <= 0.0004 deg). With
median grid spacing 1.4 deg at 600k, the nearest grid quaternion to such a
narrow basin is unlikely to have low cost -- this tests whether L-BFGS-B can
jump FROM the nearest top-50 grid point (outside the basin) INTO the basin.
The q0 axis is gradient-bearing to ~20 deg per [[basin-of-attraction]], so
L-BFGS-B may find the basin if the starting grid quaternion is within
~20 deg quaternion-space.

Pipeline per seed (verbatim from [[m127]]):
  A. SO(3) super-Fibonacci grid scan (N=600000), omega_search=-omega_true,
     mean_L1 residual vs observed_lc.
  B. L-BFGS-B polish of top-50 q0s (3-DOF rotvec), omega fixed.
...
Env: MICRO129_SEEDS, MICRO129_POOL, MICRO129_FORCE.
Out: data/results/inversion_diagnostics/m129_densegrid/seed_NNN/
     stage_a_grid.npz, stage_b_polish.npz, stage_c_hifi.npz, result.json.
Batch: batch_summary.json.

Kill criteria (see m129_REPORT.md):
  - Per-seed hard cap: 15 min wall (investigate and abort if hit).
  - Stage A watchdog: >5 min likely indicates worker crash, abort.
  - OOM during Stage A: drop POOL_SIZE 8 -> 4 and retry.
...
_env = os.environ.get('MICRO129_SEEDS', '').strip()
SEEDS = [int(s) for s in _env.split(',') if s.strip()] if _env else [POSITIVE_CONTROL_SEED]
POOL_SIZE = int(os.environ.get('MICRO129_POOL', '8'))
FORCE = os.environ.get('MICRO129_FORCE', '0') == '1'
...
OUT_BASE = RESULTS_DIR / "m129_densegrid"
...
def main():
    ...
    print(f"m129 -- densified SO(3) grid on seed 33 (flipped-omega density test)")
    ...
    # Single-seed (positive-control) hypothesis: CONFIRMED iff control hi-fi < 0.15
    control_hifi = ((control or {}).get('winner') or {}).get('hifi_mse')
    if control_hifi is None:
        verdict = 'ERROR'
    elif control_hifi < 0.15:
        verdict = 'CONFIRMED'
    else:
        verdict = 'REFUTED'
    ...
    summary = {
        ...
        'control_hifi_mse': control_hifi,
        'hypothesis_threshold_hifi': 0.15,
    ...
    print(f"  control (seed {POSITIVE_CONTROL_SEED}): "
          f"{summary['control_classification']}  hifi={control_hifi}")
    print(f"  hypothesis verdict: {verdict}  (threshold: hi-fi < 0.15)  "
