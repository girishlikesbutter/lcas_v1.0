# Series 07b — Robust Winding Enumeration

## Problem Statement

micro17's staircase finds 8 evenly-spaced winding solutions on leg 0 (peaks 183→260, dt=555.5s):
`0.32, 0.96, 1.60, 2.23, 2.88, 3.51, 4.18, 4.82 deg/s` — true ω (~2.08 deg/s) is near step 3.

But micro18's staircase on leg 1 (peaks 260→360, dt=721.4s) got:
`0.25, 3.28, 3.51, 4.00, 4.49, 4.98, 5.48, 5.98 deg/s` — **huge gap 0.25→3.28 deg/s**.
The true ω was NEVER FOUND on leg 1. This caused micro18's L-consistency test to fail.
...
## micro19 — Winding Landscape (dense |ω| sweep)
...
- Leg 0 has dense, well-separated valleys (micro17's staircase found most of them)
...
## micro20 — Multi-start Staircase (leg 1 only)
...
| Band (deg/s)  | Valid/10 | Best |ω| | micro18 found? |
|---------------|----------|---------|----------------|
...
1. **The leg-1 gap is a search failure, not physics.** Solutions exist near 2.08 deg/s
   for both legs. micro18's L-consistency failure was caused by incomplete enumeration,
