...
micro18 showed leg 1 (peaks 260→360) has a huge gap from 0.25 to 3.28 deg/s
...
- Uses 0.5 deg/s bands (or micro19 valleys if available) as targets
...
if __name__ == '__main__':
    ...
    # Compare with micro18 staircase
    micro18_mags = [0.249, 3.279, 3.509, 3.995, 4.486, 4.980, 5.477, 5.976]  # from JSON
    ...
    missed_by_staircase = [m for m in found_mags
                           if not any(abs(m - s) < 0.3 for s in micro18_mags)]
    found_near_true = any(abs(r['best_mag_degs'] - true_omega_mag_degs) < 0.3
                          for r in band_results if r['n_valid'] > 0)

    print(f"\n{'='*60}")
    print(f"micro18 staircase found:  {micro18_mags}")
    ...
    # Mark micro18 staircase solutions
    for sm in micro18_mags:
        band_idx = min(int(sm / 0.5), len(bands) - 1)
        ax.plot(-0.3, band_idx, 'b^', ms=8)
    ax.plot([], [], 'b^', ms=8, label='micro18 staircase')
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3, axis='x')

    plt.tight_layout()
    out_png = RESULTS_DIR / "micro20_multistart_staircase.png"
    plt.savefig(out_png, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\nSaved: {out_png}")

    save_results(RESULTS_DIR / "micro20_multistart_staircase.json", {
        ...
        'micro18_staircase_mags': micro18_mags,
        'bands': band_results,
        'found_near_true': found_near_true,
        'missed_by_staircase': missed_by_staircase,
        'runtime_s': round(time.time() - t0, 1),
    })
    print(f"Saved: {RESULTS_DIR / 'micro20_multistart_staircase.json'}")
