...
- **LAST COMPLETED:** Series 10 micro49-51b (pipeline generalisation, omega landscape, end-to-end with hi-fi disambiguation, 2026-03-18). Key findings:
  ...
  - **micro52 (omega magnitude):** Peak count → |omega| calibration: median 13.2% error, 90/100 within ±30%. Spectral direction matching fails (median 80°).
  - **micro53c (bridge omega):** Two-glint-circle bridge generates close omega candidates for some trajectories (traj 84: 3.6° at rank 2 by LC residual). Coverage is trajectory-dependent.
  - **micro54 (full blind pipeline):** Two-stage: bridge → LC score → top 5 omega → phi sweep → hi-fi rank. Traj 84: **omega direction 5.5°, no oracle** — first blind omega recovery. Attitude at 24.1° needs refinement. Other 4/5: bridge coverage insufficient.
- **RUNNING:** Nothing.
- **BLOCKING:** Bridge coverage — the correct omega direction appears in the top 5 LC-scored bridge candidates for ~1/5 trajectories tested. Need denser bridge sampling (finer phi grid, more hypothesis pairs) or a fundamentally different omega search.
- **NEXT STEP:** (1) Increase bridge pool coverage: finer phi grid (72 instead of 36), more candidates (10K instead of 5K). (2) After bridge gives omega within ~5°, run finer phi sweep + more NM iterations to push q0 below 5°. (3) CasADi/IPOPT multiple shooting as alternative omega search. (4) Multi-session observations for better omega constraints.
- **OPEN QUESTIONS:**
