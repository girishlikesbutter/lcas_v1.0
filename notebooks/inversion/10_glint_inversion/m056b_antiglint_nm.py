#!/usr/bin/env python3
"""Micro-56b -- Full blind pipeline with anti-glint NM cost.

Like m056 but NM cost includes anti-glint penalties at dim epochs.
This breaks the degeneracy where wrong omegas produce zero alignment cost
by accidentally aligning different normals at glint epochs.
...
Architecture (no oracle):
  ...
  Stage 2 (~5 min): For each of 200 candidates, Nelder-Mead refinement of
           (phi, omega_body) using full ODE propagation + COMBINED cost:
           (a) min-over-normals alignment at glint epochs
           (b) anti-glint penalty at dim epochs (penalize normal alignment
               where observed mag > 10)
           Parallelized with 8 workers.
...
N_PHI = 72
TOP_K_COARSE = 5000   # constant-omega pre-filter
TOP_K_ALIGN = 200     # full-ODE re-ranked
NM_MAXFEV = 500
...
_nm_glint_pabs = None
_nm_antiglint_times = None
_nm_antiglint_pabs = None
_nm_pab_anchor = None
_nm_omega_est_rad = None  # estimated omega magnitude (rad/s)
ANTI_GLINT_THRESHOLD = np.cos(np.deg2rad(8.0))  # cos(8°)
ANTI_GLINT_WEIGHT = 5.0  # penalty weight
MAG_PENALTY_WEIGHT = 50.0  # omega magnitude penalty weight


def _nm_refine_one(args):
    """Run NM refinement for one candidate.

    Cost = alignment_at_glints + ANTI_GLINT_WEIGHT * violation_at_dim_epochs.
    Returns (hyp_idx, phi, omega, cost, nfev).
    """
    ...
    ag_times = _nm_antiglint_times
    ag_pabs = _nm_antiglint_pabs
    pab_anchor = _nm_pab_anchor
    omega_est_rad = _nm_omega_est_rad

    # Pre-concatenate times for single propagation call
    all_times = np.concatenate([glint_times, ag_times])
    n_glint = len(glint_times)
    ...
    def cost_fn(params):
        ...
        try:
            all_q = propagate_sparse(qa, omega, anchor_time, all_times, I_tensor)
        except Exception:
            return 1e10

        # Component 1: alignment cost at glint epochs
        glint_q = all_q[:n_glint]
        align_cost = min_over_normals_cost(glint_q, glint_pabs, normals)

        # Component 2: anti-glint penalty at dim epochs
        ag_q = all_q[n_glint:]
        ag_penalty = 0.0
        for i in range(len(ag_q)):
            q = ag_q[i]
            R = Rotation.from_quat([q[1], q[2], q[3], q[0]]).as_matrix()
            best_dot = max(np.dot(R.T @ normals[j], ag_pabs[i])
                           for j in range(len(normals)))
            if best_dot > ANTI_GLINT_THRESHOLD:
                ag_penalty += (best_dot - ANTI_GLINT_THRESHOLD) ** 2

        # Component 3: omega magnitude penalty (prevents collapse to omega≈0)
        omega_mag = np.linalg.norm(omega)
        low = 0.4 * omega_est_rad
        high = 2.5 * omega_est_rad
        mag_pen = 0.0
        if omega_mag < low:
            mag_pen = ((low - omega_mag) / omega_est_rad) ** 2
        elif omega_mag > high:
            mag_pen = ((omega_mag - high) / omega_est_rad) ** 2

        return (align_cost + ANTI_GLINT_WEIGHT * ag_penalty
                + MAG_PENALTY_WEIGHT * mag_pen)
...
print("=" * 70)
print("m056b -- Full blind pipeline: alignment filter + anti-glint NM")
print("=" * 70)
t_global = time.time()
...
for traj_idx in TEST:
    ...
    # Stage 1a: coarse constant-omega filter → top 5000
    coarse_idx = np.argsort(best_cost)[:TOP_K_COARSE]
    print(f"  Stage 1a: {dt1:.0f}s, constant-ω filter → top {TOP_K_COARSE}")

    # Stage 1b: Re-rank top 5000 by FULL ODE alignment (accurate propagation)
    t1b = time.time()
    ode_costs = np.full(len(coarse_idx), np.inf)
    for ci_pos, ci in enumerate(coarse_idx):
        q1_wxyz = c1_wxyz[pairs_i[ci]]
        ob = best_omega_body[ci]
        try:
            gq = propagate_sparse(q1_wxyz, ob, obs_times[a1],
                                  target_times, I_tensor)
            ode_costs[ci_pos] = min_over_normals_cost(gq, target_pabs, normals)
        except Exception:
            pass
        if (ci_pos + 1) % 1000 == 0:
            print(f"    ODE re-rank [{ci_pos+1}/{len(coarse_idx)}] "
                  f"{time.time()-t1b:.0f}s", flush=True)

    dt1b = time.time() - t1b

    # Take top-200 by ODE cost
    ode_order = np.argsort(ode_costs)[:TOP_K_ALIGN]
    top_idx = coarse_idx[ode_order]
    top_hyps = h1s[pairs_i[top_idx]]
    top_phis = p1s[pairs_i[top_idx]]
    top_omega = best_omega_body[top_idx]
    top_cost = ode_costs[ode_order]

    print(f"  Stage 1b: {dt1b:.0f}s, ODE re-rank → top {TOP_K_ALIGN}, "
          f"cost range: [{top_cost[0]:.6f}, {top_cost[-1]:.6f}]")

    # ===== STAGE 2: NM refinement (parallelized) =====
    t2 = time.time()

    # Select anti-glint epochs: dim epochs (mag > 10), subsample to 30
    dim_mask = mags > 10.0
    dim_epochs = np.where(dim_mask)[0]
    if len(dim_epochs) > 30:
        ag_sel = dim_epochs[np.linspace(0, len(dim_epochs)-1, 30, dtype=int)]
    else:
        ag_sel = dim_epochs
    ag_times = obs_times[ag_sel]
    ag_pabs = pab_j2000[ag_sel]

    print(f"  Anti-glint epochs: {len(ag_sel)} (dim > 10 mag)")

    # Set shared data for NM workers (module-level globals, inherited by fork)
    _nm_normals = normals
    _nm_I = I_tensor
    _nm_anchor_time = obs_times[a1]
    _nm_glint_times = target_times
    _nm_glint_pabs = target_pabs
    _nm_antiglint_times = ag_times
    _nm_antiglint_pabs = ag_pabs
    _nm_pab_anchor = pab_j2000[a1]
    _nm_omega_est_rad = np.deg2rad(omega_est)
...
print("\n" + "=" * 70)
print("SUMMARY — m056b (alignment + anti-glint NM, fully blind)")
print("=" * 70)
...
# Save
json_path = RESULTS_DIR / "m056b_antiglint_nm.json"
with open(str(json_path), 'w') as f:
    json.dump({
        'experiment': 'm056b_antiglint_nm',
        'config': {
