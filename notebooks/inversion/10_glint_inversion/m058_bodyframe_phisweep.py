#!/usr/bin/env python3
"""Micro-58 -- Full blind pipeline: body-frame bridge → LC → phi sweep → hi-fi.

Combines the body-frame bridge discovery (m057) with the proven phi sweep
+ hi-fi disambiguation (m051b).
...
Stage 3 (~15s):  Per omega: phi sweep (10 hyp × 36 phi) → top 4 by alignment → lo-fi LC → top 2
Stage 4 (~5min): Hi-fi LC on all Stage 3 finalists → pick best

No oracle. 10 trajectories from m046 dataset.
"""
...
print("=" * 70)
print("m058 -- Full blind: body-frame bridge → LC → phi sweep → hi-fi")
print("=" * 70)
t_global = time.time()
...
for traj_idx in TEST:
    ...
    for oi, (omega_at_anchor, _) in enumerate(omega_cands):
        ...
        for cost, hi, phi in hyp_results[:4]:
            ...
            try:
                ...
                rv = Rotation.from_quat(
                    [q0c[1], q0c[2], q0c[3], q0c[0]]).as_rotvec()
                lofi = float(_obj_lo_global.evaluate(np.concatenate([rv, o0c])))
                stage3_finalists.append((q0c, o0c, lofi, oi, hi))
            except Exception:
                pass

    # Sort by lo-fi, keep top 10 (2 per omega candidate max)
    ...
    for q0c, o0c, lofi, oi, hi in top_finalists:
        try:
            rv = Rotation.from_quat(
                [q0c[1], q0c[2], q0c[3], q0c[0]]).as_rotvec()
...
print("\n" + "=" * 70)
print("SUMMARY — m058 (body-frame bridge → LC → phi sweep → hi-fi)")
print("=" * 70)
...
fig.suptitle("Micro-58: Body-frame bridge + phi sweep (fully blind)",
...
plt.tight_layout()
plot_path = RESULTS_DIR / "m058_bodyframe_phisweep.png"
fig.savefig(str(plot_path), dpi=150, bbox_inches='tight')
plt.close(fig)
print(f"\nPlot: {plot_path}")

# Save JSON
json_path = RESULTS_DIR / "m058_bodyframe_phisweep.json"
with open(str(json_path), 'w') as f:
    json.dump({
        'experiment': 'm058_bodyframe_phisweep',
        'config': {
