...
Method:
  ...
  each seed, dedup converged candidates, and compare to micro10's 5643
...
LOG_PATH = RESULTS_DIR / "micro38_stdout.txt"
...
# ===========================================================================
# Lo-fi brightness and iso-brightness objective (same pattern as micro10)
# ===========================================================================
...
# ===========================================================================
# Dedup via hierarchical clustering (same approach as micro10)
# ===========================================================================
...
if __name__ == '__main__':
    ...
    # -----------------------------------------------------------------------
    # 8. Load micro10 candidates and cross-compare
    # -----------------------------------------------------------------------
    print("\n--- Loading micro10 candidates for comparison ---")
    micro10_path = RESULTS_DIR / "micro10_optimized_candidates.npz"
    micro10_data = np.load(str(micro10_path))
    micro10_q_wxyz = micro10_data['candidate_q_wxyz']   # (5643, 4) wxyz
    micro10_ang_dists = micro10_data['ang_dists_to_truth']
    n_micro10 = len(micro10_q_wxyz)
    print(f"Loaded {n_micro10} micro10 candidates")

    micro10_nearest = micro10_ang_dists.min()
    print(f"micro10 nearest to truth@q0: {micro10_nearest:.2f} deg")

    # Cross-match: for each micro10 candidate, find nearest PAB-seeded candidate
    print("\n--- Cross-matching micro10 vs PAB-seeded candidates ---")
    t_cross = time.time()

    # Pairwise angular distances between micro10 and PAB-seeded reps
    R_micro10 = Rotation.from_quat(micro10_q_wxyz[:, [1, 2, 3, 0]])
    ...
    print(f"Cross-match within {CROSS_MATCH_DEG} deg: "
          f"{n_matched}/{n_micro10} micro10 candidates recovered "
    ...
    print()
    print(f"micro10 (random SO(3)) comparison:")
    print(f"  Seeds:                10,000")
    print(f"  Unique candidates:    {n_micro10}")
    print(f"  Nearest to truth@q0:  {micro10_nearest:.2f} deg")
    print()
    print(f"Cross-match ({CROSS_MATCH_DEG} deg):")
    print(f"  micro10 candidates recovered by PAB-seeded: "
    ...
    axes[0].hist(
        micro10_ang_dists, bins=bins_dist, alpha=0.4, color='gray',
        label=f'micro10 random (N={n_micro10})',
    )
    axes[0].axvline(
    ...
    axes[0].axvline(
        micro10_nearest, color='gray', linestyle=':', linewidth=1.5,
        label=f'micro10 nearest = {micro10_nearest:.1f} deg',
    )
    ...
    axes[1].set_ylabel('Fraction of micro10 candidates', fontsize=11)
    axes[1].set_title('Cross-match: micro10 recovery', fontsize=12)
    ...
    plot_path = RESULTS_DIR / "micro38_pab_seeded_candidates.png"
    ...
    npz_path = RESULTS_DIR / "micro38_pab_seeded_candidates.npz"
    np.savez(
        npz_path,
    ...
    # JSON summary
    json_results = {
        'experiment': 'micro38_pab_seeded_candidates',
        'target_epoch': TARGET_EPOCH,
        ...
        'micro10_comparison': {
            ...
            'micro10_nearest_to_truth_deg': round(float(micro10_nearest), 2),
    ...
    json_path = RESULTS_DIR / "micro38_pab_seeded_candidates.json"
