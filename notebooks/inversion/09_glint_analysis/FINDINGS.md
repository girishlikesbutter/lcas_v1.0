...
> **Status: DONE — validated building blocks.** Specular glint physics fully characterized. mag < 6 = 100% specular. PAB-circle 1-DOF constraint. Classifier F1=0.90. Geometric filter. Two datasets (m046, m048). All reusable.
...
1. **Series 09a (m034-40):** Established that brightness peaks are specular glints, quantified PAB alignment, tested PAB-circle seeding, demonstrated component identification from magnitude.
2. **Series 09b (m044-48):** Deep classification — built visualization tools, generated 200 trajectory datasets at realistic omega, quantified specular vs diffuse thresholds, built ML classifier and geometric filter for inversion.
...
Corrected to [0.1, 1.5] deg/s for m046/48 datasets. Key implications:
...
## Glint-Anchored NLP Inversion (m042b)

First successful attitude+omega recovery from glint constraints using a two-phase approach.

**Architecture:**
1. **Phase 1 — Coarse phi sweep:** For each of 10 glint-producing normals as anchor hypothesis, sweep 36 phi values (every 10°) with oracle omega. Score with three-component cost: glint alignment (min-over-normals at 9 glint epochs) + anti-glint violations (count of dim epochs with alignment > cos(8°)) + lo-fi LC residual (10 sampled epochs). Correct hypothesis ranks #1 in 75 seconds.
2. **Phase 2 — Local refinement:** From the best (hypothesis, phi), run Nelder-Mead on 4-DOF (phi + omega). Converges to 1.77° attitude error, 1.15° omega direction error.

**Why direct NLP fails:** The min-over-normals cost has many false phi basins where wrong normals coincidentally align at different glint epochs. Nelder-Mead finds good omega (2.8° dir) but gets trapped at wrong phi (37° error, always near phi=0). The phi sweep bypasses this by brute-force enumeration.

**Anti-glint constraint:** At epochs where observed mag > 11, the true trajectory has zero instances of any glint-producing normal aligned within 8° of PAB. Wrong hypotheses have 2-22 such violations. Must use only the 10 glint-producing normals (not all 14) and cos(8°) threshold (not 15°) — the 4 tiny dish-edge normals (1.1 m²) can align with PAB without producing observable glints.

**Antiparallel degeneracy:** G1 and G12 (same family, opposite faces) produce identical glint cost. The lo-fi LC residual breaks this: correct solution = 0.057, antiparallel twin = 3.0 (50x gap). The satellite's asymmetric geometry (dishes, panels) produces different brightness at non-glint epochs.

**Remaining blocker:** Only oracle omega start converges. 10% perturbation fails. The omega direction basin is ~2°. Need omega initialisation strategy.
...
|---------|----------|-------------|-------------|------------|-------------|------|
| m046 | `m046_trajectories/` | 100 | [0.11, 1.48] | Fixed 10:00 UTC | ~9-17° | 13.5 MB |
| m048 | `m048_trajectories/` | 100 | [0.11, 1.48] | Random 08:35-15:35 | 0-97° | 19.0 MB |

Both store per trajectory: quaternions, k1/k2, PAB (body + J2000 + equatorial in m048), phase angles, hi-fi + lo-fi magnitudes, per-group flux + fractional flux, alignment angles, peak detection.
...
| File | Purpose |
|------|---------|
| `m034_pab_alignment.py` | PAB alignment diagnostic (oracle) |
| `m035_multi_trajectory_pab.py` | 30-trajectory robustness |
| `m036_pab_candidate_filter.py` | PAB as iso-brightness post-filter |
| `m037_brdf_glint_profile.py` | BRDF specular lobe characterisation |
| `m038_pab_seeded_candidates.py` | PAB-circle seeding (failed) |
| `m039_glint_identification.py` | Component ID from glint shape |
| `m041_glint_classification.py` | 30-trajectory glint classification |
| `m042_glint_anchored_nlp.py` | Glint-anchored 4-DOF NLP (partial — validated cost function, multi-start OOM) |
| `m042b_anchor_hypothesis_sweep.py` | **Two-phase proof-of-concept: phi sweep → refinement → 1.77° att, 1.15° omega** |
| `m043_focused_pab_seeding.py` | Focused single-normal PAB seeding (failed — 12.4° vs 0.69° random) |
| `m044_normal_sphere.py` | **Normal-family sphere visualization (multi-mode)** |
| `m045_glint_filter_basin.py` | Glint filter basin shape analysis |
| `m046_generate_trajectories.py` | 100 fixed-time trajectory dataset |
| `m047_glint_statistics.py` | Exhaustive glint statistics + ML |
| `m048_generate_trajectories_v2.py` | 100 varied-time trajectory dataset |
