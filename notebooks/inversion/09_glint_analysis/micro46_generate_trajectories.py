#!/usr/bin/env python3
"""Micro-46 -- Generate 100 realistic trajectories with full diagnostic data.

Omega magnitude: uniform [0.1, 1.5] deg/s (realistic for retired GEO).
Random q0 (uniform SO(3)), random omega direction.

Stores per trajectory:
  - Identity: seed, q0, omega (cartesian + spherical)
  - Attitude: quaternion history
  - Geometry: k1_body, k2_body, pab_body, pab_j2000, obs_dist
  - Lightcurves: mag_hifi, mag_lofi
  - Flux: group_flux (G, N), group_frac_flux (G, N)
  - Alignment: ang_dist (G, N), min_ang_dist (N), best_group (N)
  - Peaks: hifi_peak_epochs, hifi_peak_prominences

All stored in one NPZ + shared metadata.
"""
...
RESULTS_DIR = PROJECT_ROOT / "data" / "results" / "inversion_diagnostics" / "micro46_trajectories"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

# ===== Configuration =====
N_TRAJECTORIES = int(sys.argv[1]) if len(sys.argv) > 1 else 100
N_OBS = 500
OMEGA_MAG_RANGE_DPS = (0.1, 1.5)  # realistic for retired GEO
GROUP_ORDER = [13, 0, 8, 5, 7, 6, 11, 2, 12, 1]  # +X,-X,+Y,-Y,+Z,-Z,+WD,-WD,+ED,-ED
GROUP_NAMES = ['+X', '-X', '+Y', '-Y', '+Z', '-Z', '+WD', '-WD', '+ED', '-ED']

# ===========================================================================
# One-time setup (shared geometry)
# ===========================================================================
print("=" * 70)
print(f"micro46 -- Generate {N_TRAJECTORIES} realistic trajectories")
print(f"  omega range: {OMEGA_MAG_RANGE_DPS} deg/s")
print("=" * 70)
t_global = time.time()

print("Setting up shared geometry...")
...
art_normals, _ = apply_articulation_to_arrays(facet_arrays, art_matrices, 0, satellite)
all_unique_normals, inverse_indices = np.unique(
    np.round(art_normals, 4), axis=0, return_inverse=True)
unique_normals = all_unique_normals[GROUP_ORDER]  # (G=10, 3)
n_groups = len(GROUP_ORDER)

# Build group-to-facet mapping for flux aggregation
group_facet_masks = []
...
print(f"  {N_OBS} epochs, dt={dt_sampling:.2f}s, {n_groups} normal groups")
...
def process_trajectory(seed):
    """Generate all data for one trajectory. Returns dict of arrays."""
    t0 = time.time()
...
# ===========================================================================
# Run trajectories — save each individually, skip completed
# ===========================================================================
TRAJ_DIR = RESULTS_DIR / "per_trajectory"
TRAJ_DIR.mkdir(parents=True, exist_ok=True)

def run_and_save(seed):
    """Run one trajectory and save to individual NPZ. Skip if exists."""
    out_path = TRAJ_DIR / f"traj_seed{seed:03d}.npz"
    if out_path.exists():
        print(f"  [skip] seed={seed:3d} (already exists)", flush=True)
        return seed, True

    result = process_trajectory(seed)
    np.savez_compressed(str(out_path),
        **{k: v for k, v in result.items() if isinstance(v, (np.ndarray, float, int))})
    om = result['omega_mag_dps']
    n_peaks = len(result['hifi_peak_epochs'])
    rt = result['runtime_s']
    print(f"  [done] seed={seed:3d}  |omega|={om:.2f} dps  "
          f"peaks={n_peaks:3d}  {rt:.1f}s", flush=True)
    return seed, False

# Check if this is a single-seed worker invocation
if os.environ.get('MICRO46_WORKER_SEED') is not None:
    worker_seed = int(os.environ['MICRO46_WORKER_SEED'])
    run_and_save(worker_seed)
    sys.exit(0)

print(f"\nRunning {N_TRAJECTORIES} trajectories (8 parallel subprocesses)...")

import subprocess

# Launch in batches of 8
seeds_todo = []
...
print(f"  {len(seeds_todo)} to compute, {N_TRAJECTORIES - len(seeds_todo)} already done", flush=True)
...
for batch_start in range(0, len(seeds_todo), BATCH_SIZE):
    ...
    for seed in batch:
        env = os.environ.copy()
        env['MICRO46_WORKER_SEED'] = str(seed)
    ...
    for seed, p in procs:
        ...
        # Print the [done] line from the worker
...
# ===========================================================================
# Collect all per-trajectory NPZs into one master file
# ===========================================================================
print("\nCollecting per-trajectory files into master NPZ...")
results = []
for seed in range(N_TRAJECTORIES):
...
metadata = {
    ...
    'omega_mag_range_dps': np.array(OMEGA_MAG_RANGE_DPS),
...
# Per-trajectory arrays (stacked)
n_traj = len(results)
seeds = np.array([r['seed'] for r in results])
q0s = np.array([r['q0_wxyz'] for r in results])                  # (T, 4)
omega0s = np.array([r['omega0_rad'] for r in results])            # (T, 3)
omega_mags = np.array([r['omega_mag_dps'] for r in results])      # (T,)
omega_thetas = np.array([r['omega_theta_deg'] for r in results])  # (T,)
omega_phis = np.array([r['omega_phi_deg'] for r in results])      # (T,)
all_quats = np.array([r['quaternions'] for r in results])         # (T, N, 4)
all_k1 = np.array([r['k1_body'] for r in results])               # (T, N, 3)
all_k2 = np.array([r['k2_body'] for r in results])               # (T, N, 3)
all_pab_body = np.array([r['pab_body'] for r in results])         # (T, N, 3)
all_mag_hifi = np.array([r['mag_hifi'] for r in results])         # (T, N)
all_mag_lofi = np.array([r['mag_lofi'] for r in results])         # (T, N)
all_group_flux = np.array([r['group_flux'] for r in results])     # (T, G, N)
all_group_frac = np.array([r['group_frac_flux'] for r in results])# (T, G, N)
all_ang_dist = np.array([r['ang_dist'] for r in results])         # (T, G, N)
all_min_ang = np.array([r['min_ang_dist'] for r in results])      # (T, N)
all_best_grp = np.array([r['best_group'] for r in results])       # (T, N)
runtimes = np.array([r['runtime_s'] for r in results])

# Peak data is ragged (different n_peaks per trajectory) — store as object arrays
# or save separately. Use a flat approach: concatenate with trajectory index.
all_peak_seeds = []
all_peak_epochs = []
all_peak_proms = []
for r in results:
    n_pk = len(r['hifi_peak_epochs'])
    all_peak_seeds.extend([r['seed']] * n_pk)
    all_peak_epochs.extend(r['hifi_peak_epochs'].tolist())
    all_peak_proms.extend(r['hifi_peak_prominences'].tolist())

peak_seeds = np.array(all_peak_seeds, dtype=int)
peak_epochs = np.array(all_peak_epochs, dtype=int)
peak_proms = np.array(all_peak_proms, dtype=float)

# Save
npz_path = RESULTS_DIR / "micro46_trajectories.npz"
...
print(f"\n{'=' * 70}")
print(f"micro46 complete: {N_TRAJECTORIES} trajectories in {elapsed:.0f}s ({elapsed/60:.1f}min)")
print(f"  omega range: [{omega_mags.min():.2f}, {omega_mags.max():.2f}] deg/s")
print(f"  mean runtime per trajectory: {runtimes.mean():.1f}s")
print(f"  total peaks: {len(peak_seeds)}")
print(f"  mean peaks per trajectory: {len(peak_seeds)/N_TRAJECTORIES:.1f}")
print(f"{'=' * 70}")
