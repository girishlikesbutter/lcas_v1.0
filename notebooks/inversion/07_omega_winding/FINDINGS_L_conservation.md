...
> **Status: DONE — validated building block.** L-conservation uniquely identifies the correct winding pair with 9 orders of magnitude gap. Robust to 10° endpoint error. Reusable in any pipeline that generates multi-winding bridge candidates.
...
**The blocking problem is not L-consistency itself, but ensuring the staircase on each leg finds the correct winding.** m018 failed because leg 1's staircase jumped over the true winding. The fix is to ensure denser staircase coverage, or to run the staircase with better initial guesses that don't miss windings.
