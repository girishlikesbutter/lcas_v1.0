#!/usr/bin/env python3
"""
Micro-11 — Joint pipeline with dense candidates (13,848 from 10M sampling).

Micro-08 proved the joint filter works but truth wasn't recoverable (nearest
candidate 5.95 deg away). Micro-10b generated 13,848 candidates from 10M random
SO(3), nearest 2.14 deg from truth. Now test whether the joint pipeline can
recover truth with these better seeds.

Pipeline per candidate:
  1. Scan omega on plane perp to brightness gradient (15x15 = 225 trials)
  2. Propagate to peak 2 with Euler dynamics, brightness filter (3%)
  3. Derivative filter |g2·w2| < threshold (0.1, 0.5, 1.0)
  4. Propagate survivors to peak 3, brightness filter (3%)
"""
...
if __name__ == '__main__':
    ...
    # Load dense candidates from micro-10b
    ckpt = np.load(RESULTS_DIR / 'micro10b_dense_candidates.npz')
    ...
    print(f"Candidates: {M} (nearest to truth: {ang_dists.min():.2f} deg, "
          f"{(ang_dists < 5).sum()} within 5 deg)")
    ...
    with get_context('fork').Pool(N_WORKERS) as pool:
        for result in pool.imap_unordered(process_candidate, range(M), chunksize=20):
            all_survivors.extend(result)
            done += 1
            if done % 500 == 0 or done == M:
                el = time.time() - t1
                rate = done / el if el > 0 else 1
                print(f"  [{done:>6}/{M}] {len(all_survivors)} B-survivors "
    ...
    print(f"\nBrightness filter (peak2, {TOL_B_PCT}%): {n_bs} survivors [{time.time()-t1:.0f}s]",
          flush=True)
    if n_bs == 0:
        print("No survivors — pipeline complete.")
        sys.exit(0)
    ...
    print(f"\n{'='*78}")
    print(f"JOINT FILTER RESULTS — micro-11 (dense candidates)")
    print(f"{'='*78}")
    ...
    for th in DERIV_THRESHOLDS:
        ...
        if n_j > 0:
            ...
            print(f"  {'#':>3} {'Ci':>6} {'AE1':>7} {'AE2':>7} {'|w|d/s':>7} "
            ...
            for r in range(n_show):
                ...
                print(f"  {r+1:>3} {s_ci[si]:>6} {ae1:>7.1f} {ae2:>7.1f} {omd:>7.2f} "
                      f"{oerr:>7.2f} {s_res2[si]:>7.4f} {res3[jidx[order[r]]]:>7.4f} "
                      f"{gdot[si]:>7.3f}")

    # ── Save checkpoint ──
    np.savez(RESULTS_DIR / 'micro11_dense_joint.npz',
