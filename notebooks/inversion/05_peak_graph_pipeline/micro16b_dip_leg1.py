#!/usr/bin/env python3
"""Micro-16b — Dip (trough) constraint experiment, leg 1.

Same structure as micro16 but tests leg 1: peak 260 → peak 360.
Leg 0 had no interior trough (monotonically declining); leg 1 may have one.

Scientific question: Does the trough on leg 1 discriminate the true path?
"""
...
if __name__ == '__main__':
    ...
    # ── Find trough on leg 1 ──────────────────────────────────────────
    PEAK_A = int(PEAKS[1])   # 260
    PEAK_B = int(PEAKS[2])   # 360
    LEG_LEN = PEAK_B - PEAK_A  # 100
    ...
    # Check if trough is genuinely interior
    interior = (trough_offset > 0) and (trough_offset < LEG_LEN)
    if not interior:
        print(f"WARNING: No interior trough found on leg 1 — trough at boundary "
              f"(offset={trough_offset}, leg_len={LEG_LEN})")

    print(f"Trough epoch: {TROUGH_IDX} (offset={trough_offset}, t_from_peak_A = {t_trough:.1f}s)")
    print(f"Interior trough: {interior}")
    ...
    N1, N2 = len(CANDS[1]), len(CANDS[2])
    all_indices = [(i, j) for i in range(N1) for j in range(N2)]
    rng.shuffle(all_indices)

    true_pair = (int(TRUTH_IDX[1]), int(TRUTH_IDX[2]))
    sample_pairs = [true_pair] + [p for p in all_indices if p != true_pair][:N_PAIRS]

    args_list = [
        (i, j, CANDS[1][i], BW[1][i, j], t_trough, TROUGH_IDX, obs_brightness_at_trough)
    ...
    ax = axes[0]
    rand_valid_scores = all_scores[valid_mask][1:]
    ...
    ax.set_ylabel('Count')
    interior_str = "interior" if interior else "BOUNDARY"
    ax.set_title(f'Leg 1 trough brightness error — hi-fi scoring\n'
                 f'Trough epoch {TROUGH_IDX} ({interior_str}), rank: {rank}/{n_valid}')
    ...
    ax2.set_title('Leg 1: Predicted trough brightness vs target')
    ax2.legend(fontsize=9)

    plt.tight_layout()
    fig_path = RESULTS_DIR / "micro16b_dip_leg1.png"
    ...
    json_path = RESULTS_DIR / "micro16b_dip_leg1.json"
    ...
    interior_warn = "" if interior else "\n*** WARNING: trough at boundary, not interior ***"
    print(f"""
=== micro16b: dip constraint experiment — leg 1 ===
Leg 1: peak {PEAK_A} → peak {PEAK_B}
Trough at epoch: {TROUGH_IDX} (offset={trough_offset}/{LEG_LEN}, t_from_peak_A = {t_trough:.1f}s)
Interior trough: {interior}{interior_warn}
...
VERDICT: {verdict}
  (micro14 baseline: truth rank ~13,000/121,326)
