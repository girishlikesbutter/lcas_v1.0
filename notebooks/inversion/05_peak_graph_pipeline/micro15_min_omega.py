#!/usr/bin/env python3
"""Micro-15 — minimum-magnitude ω hypothesis test.

Does using a zero initial guess + min-|ω| objective find a smaller ω than the
axis-angle estimate (micro13 method)?  Does the true pair stand out?
"""
...
ALPHA = 0.1  # weight on ||ω||² relative to arrival error
...
CANDS     = [stage1["c0"], stage1["c1"], stage1["c2"]]  # each [50,4] wxyz
TRUTH_IDX = stage1["truth_idx"]    # [i0, i1, i2]
BDT       = stage2["bdt"]          # [dt_01, dt_12] seconds
...
def bridge_axis_angle(q_start, q_end, dt, I):
    """Bridge ω via axis-angle initial guess (current micro13 method)."""
...
def bridge_min_omega(q_start, q_end, dt, I):
    """Min-|ω| bridge via zero initial guess.
    Objective: arrival_error^2 + ALPHA * ||ω||^2
    """
    w0    = np.zeros(3)
    times = np.array([0.0, dt])
    def obj(w):
...
if __name__ == "__main__":
    ...
    err_min_all = np.array([r['err_min'] for r in results])

    true_omega_mag = float(np.linalg.norm(CTX.true_omega0))  # rad/s
    ...
    # Panel 1: scatter mag_aa vs mag_min
    ...
    ax1.set_ylabel('|ω| zero start / min-|ω| (deg/s)')
    ax1.set_title('ω magnitude: axis-angle vs min-|ω| start')
    ax1.legend(fontsize=8)

    # Panel 2: sorted mag_min
    sorted_idx = np.argsort(mag_min_degs)
    sorted_mags = mag_min_degs[sorted_idx]
    true_rank   = int(np.where(sorted_idx == 0)[0][0])  # position of true pair (index 0)
    ranks       = np.arange(len(sorted_mags))

    ax2.plot(ranks, sorted_mags, color='blue', lw=1.2, label='all pairs')
    ...
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "micro15_min_omega.png", dpi=150)
    plt.close()
    ...
    verdict = ("hypothesis supported"
               if rank_by_mag_min <= max(1, len(results) // 10)
               else "not supported")
    print("=== micro15: minimum-magnitude ω experiment ===")
    print(f"Leg 0: peak {PEAKS[0]} → peak {PEAKS[1]}, dt = {dt:.1f} s")
    print(f"True ω magnitude: {np.degrees(true_omega_mag):.2f} deg/s")
    print("True pair:")
    print(f"  axis-angle start |ω|:  {true_mag_aa_degs:.2f} deg/s  (err: {true_result['err_aa']:.3f} deg)")
    ...
    print(f"Random pairs (N={len(random_results)}):")
    print(f"  mean |ω_min|: {np.degrees(np.mean([r['mag_min'] for r in random_results])):.2f} deg/s, "
          f"std: {np.degrees(np.std([r['mag_min'] for r in random_results])):.2f} deg/s")
    print(f"  mean |ω_aa|:  {np.degrees(np.mean([r['mag_aa']  for r in random_results])):.2f} deg/s, "
          f"std: {np.degrees(np.std([r['mag_aa']  for r in random_results])):.2f} deg/s")
    print(f"VERDICT: {verdict}")
    print(f"Total runtime: {runtime_s:.1f} s")

    save_results(RESULTS_DIR / "micro15_min_omega.json", results_out)
