...
Diagnosis from micro16: the axis-angle bridge ω is optimised only for
...
if __name__ == '__main__':
    ...
    fig.suptitle(f'micro16c: trough-constrained bridge (α_trough={ALPHA_TROUGH})', fontsize=11)
    ...
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "micro16c_trough_constrained.png", dpi=150)
    plt.close()
    ...
    save_results(RESULTS_DIR / "micro16c_trough_constrained.json", results_out)

    # ── Console summary ───────────────────────────────────────────────────────
    verdict_obj = ("DISCRIMINATES" if rank <= max(5, len(results) // 10)
                   else "no signal")
    print(f"""
=== micro16c: trough-constrained bridge ===
