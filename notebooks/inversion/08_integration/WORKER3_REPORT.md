# Worker 3 Report: micro28 — Integration Pipeline with Nudged Attitudes

## What was attempted

**Task:** Run the micro26 pipeline (band-sweep enumeration + L-conservation filter) with attitudes perturbed by 1, 2, 5 degrees from truth. 10 trials per nudge level (30 total).

**Scripts written:**
- `notebooks/inversion/08_integration/micro28_integration_nudged.py` — multiple iterations

### Iteration history

1. **v1 (committed as `ecc1193` on `exp/integration-nudged`):** Direct port of micro26 with nudge loop. Full tumbling-mode bridge (13 bands × 10 starts = 130 solves per leg, Pool(8)). First trial took **~1080s** — extrapolates to ~9 hours for 30 trials. Killed.
...
4. **v4 (oracle sweep + nudged refinement, current on disk):** One full tumbling oracle sweep (like micro26), then short warm-started refinement (maxiter=30) for each nudged trial. Oracle sweep was still running when killed (~15 min per leg expected). This is the most promising approach but was not completed.

## What results exist

- `data/results/inversion_diagnostics/micro28_integration_nudged.json` — from v2 (PA-only), all 30 trials MISS. **Not useful.**
- `data/results/inversion_diagnostics/micro28_integration_nudged.png` — from v2, shows 0% accuracy across all nudge levels. **Not useful.**
...
- **Uncommitted:** v4 script is on disk at `notebooks/inversion/08_integration/micro28_integration_nudged.py` (oracle sweep + nudged refinement approach)
- **Nothing running.**
