#!/usr/bin/env python3
"""Micro-26 — End-to-end integration: band-sweep enumeration + L-conservation filter.
Wires micro20 (band-sweep bridge solves) + micro23 (L-conservation scoring)
using oracle attitudes at 3 peaks.
"""
...
ARRIVAL_THRESH, DEDUP_TOL = 1e-6, 0.05  # deg/s
...
if __name__ == '__main__':
    ...
    true_k, true_j = -1, -1
    true_mag_A, true_mag_B = np.rad2deg(np.linalg.norm(om_A)), np.rad2deg(np.linalg.norm(om_B))
    L_leg0 = []
    for k, c in enumerate(leg0):
        ...
        if abs(c['mag_degs'] - true_mag_A) < DEDUP_TOL: true_k = k
    L_leg1 = []
    for j, c in enumerate(leg1):
        L_leg1.append(compute_L(q_B, np.array(c['omega']), I))
        if abs(c['mag_degs'] - true_mag_B) < DEDUP_TOL: true_j = j
    ...
    de = lambda a, b: float(np.rad2deg(np.arccos(np.clip(np.dot(a,b)/(np.linalg.norm(a)*np.linalg.norm(b)+1e-30),-1,1))))
    dir0, dir1 = de(best_om0, om_A), de(best_om1, om_B)
    ...
    fig.suptitle("Micro-26: Integration pipeline (oracle attitudes)", fontsize=12)
    ...
    mags0, mags1, x, bw = [c['mag_degs'] for c in leg0], [c['mag_degs'] for c in leg1], np.arange(max(n0,n1)), 0.35
    ...
    out_png = RESULTS_DIR / "micro26_integration_oracle.png"
    plt.savefig(out_png, dpi=150, bbox_inches='tight'); plt.close(); print(f"Saved: {out_png}")
    save_results(RESULTS_DIR / "micro26_integration_oracle.json", {
    ...
    print(f"Saved: {RESULTS_DIR / 'micro26_integration_oracle.json'}")
