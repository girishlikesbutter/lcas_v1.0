...
> **Status: CLOSED.** Bridge solver coverage is the bottleneck — 10 starts/band insufficient at dt=721s. The true omega is absent from leg 1's candidate set. This was never resolved because the research pivoted to glint-based approaches (Series 09). The bridge+L-conservation pipeline was never tested end-to-end with sufficient starts. See `DEAD_ENDS.md`.

## Micro-26: End-to-end integration with oracle attitudes

**Question:** Does wiring band-sweep enumeration (m020) + L-conservation filter (m023)
...
In m023, this worked because candidates were constructed along the true omega direction
...
**Next steps (from m026):**
- m026c: Direction-aware dedup → see below
- m027: Bridge screening feasibility → infeasible (11s/solve, no cheap alternative)
- m028: Nudged attitudes → PA-mode fundamentally wrong, v4 incomplete
...
**Setup:** Same as m026, but dedup now requires both magnitude AND direction proximity to merge candidates. 10 random starts per band, 13 bands.
...
- m026 (mag-only dedup): 34/32 candidates, rank 423/1088
- m026c (direction-aware): 54/49 candidates, true omega absent from leg 1
...
- Leg 1: true omega not found (same issue as m026c — only 5 starts per band here)
...
**Verdict:** Inconclusive. 1000 seeds is too few — we already know from m010 that 10K seeds bring nearest within ~1° at all peaks. The failures at peaks 183/260 are seed coverage failures, not hi-fi pruning failures. Needs re-run with 10K seeds to properly evaluate.

---

## Summary: Bridge Solver Coverage is THE Bottleneck (2026-03-12)

All three experiments (m026c, m029, m030) point to the same conclusion: **the pipeline's blocking failure mode is that the bridge solver cannot reliably find the true omega direction on longer legs.**
