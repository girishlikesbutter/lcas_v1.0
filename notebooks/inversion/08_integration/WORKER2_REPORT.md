...
**Goal:** micro27 — Single-bridge screening for candidate pair pruning. Test whether
...
`notebooks/inversion/08_integration/micro27_bridge_screening.py` — 170 lines.

- Part A: 50 oracle-based candidates per peak (oracle + 1-3 deg random nudge),
  2500 pairs per leg, 2 legs.
- Part B: 50 micro13 iso-brightness candidates per peak, same structure.
...
- **Script:** `notebooks/inversion/08_integration/micro27_bridge_screening.py` (uncommitted)
...
- **Running:** Nothing.
- **Uncommitted:** `micro27_bridge_screening.py` script (functional but infeasible
  at N=50 due to solve cost), `WORKER_REPORT.md` (from prior micro28 work).
