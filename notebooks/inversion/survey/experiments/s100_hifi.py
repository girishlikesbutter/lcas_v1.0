"""s100_hifi — hi-fi rho-band classification of the s100 5-step inversion winners.

Mirror of s099_hifi but reads results/s100/seed{SEED}/invert.npz (same schema).
Renders each blind candidate hi-fi to classify rho-bands, plus a truth control.
Each candidate's t=0 state is back-propagated from (q_a, omega_a) at ep_a
(times[0]==0 gauge). Serial trimesh renders, v2-independent.

Run ONLY after inspecting invert.json — surrogate-first discipline says don't
hi-fi-render 15 candidates whose best surrogate full-RMSE is already junk.
Use S100_HIFI_N to limit renders (default 15).
"""
import os
...
def main():
    ...
    nlim = int(os.environ.get("S100_HIFI_N", 15))
    ...
    print(f"===== s100_hifi BLIND winners | seed {SEED} | {len(qa)+1} renders | floor {floor:.4f} =====", flush=True)
