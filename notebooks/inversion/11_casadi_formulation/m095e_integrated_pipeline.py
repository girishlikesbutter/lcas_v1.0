#!/usr/bin/env python3
"""
m095e — Integrated Pipeline: NM200 + Geo + Multi-Window Hi-Fi.

Changes from m077/90 baseline:
  1. NM_TOP=200 (rescues truth from deep grid ranks)
  2. GEO_TOP=20 (geo-refine top 20 deduped NM candidates, not all)
  3. Replace lo-fi re-ranking (Step 4b) with multi-window hi-fi
  4. Majority vote across 180s/360s/720s for selection

Tested in m095a-d:
  - NM_TOP=200 rescues seed 27 (grid#149 → NM#18)
  - Geo refinement must be KEPT (seed 93 regressed without it)
  - Short-window hi-fi discriminates better than lo-fi (no phantom peaks)
  - Lo-fi re-ranking is harmful (phantom peaks from no-shadow lo-fi)

Usage:
  MICRO95E_SEED=27 python3 m095e_integrated.py
"""
...
TRAJ_SEED = int(os.environ.get('MICRO95E_SEED', '27'))
...
N_PHI_COARSE = 36
N_PHI_FINE = 360
NM_TOP = int(os.environ.get('MICRO95E_NM_TOP', '200'))
GEO_TOP = 20
LOFI_TOP = int(os.environ.get('MICRO95E_LOFI_TOP', '200'))
PEAK_WINDOW = 3
...
CKPT_DIR = Path(os.environ.get('MICRO95E_CKPT_DIR',
                str(RESULTS_DIR / f"m095e_integrated_seed{TRAJ_SEED:03d}")))
CKPT_DIR.mkdir(exist_ok=True)
_log_file = open(str(CKPT_DIR / "pipeline.log"), "w")
sys.stdout = Tee(sys.__stdout__, _log_file)

print("=" * 60, flush=True)
print(f"m095e — Integrated (seed {TRAJ_SEED})")
