#!/usr/bin/env python3
"""
m082 — Brightness-match cost pipeline.

Replaces the alignment cost (1-dot)^2 with a brightness-match cost
(predicted_mag - observed_mag)^2 everywhere: grid phi sweep and
geometric refinement.

The alignment cost is degenerate because +X/+WD/+ED normals are 15°
apart (dot=0.966). When one aligns, the others co-align and score
nearly identically. But they produce very different MAGNITUDES:
  ±X at perfect alignment → mag 4.93  (116 m² effective area)
  ±WD/±ED              → mag 7.43  (dish, small area)
The brightness-match cost exploits this 2.5 mag gap.

Calibration: for each normal, compute hi-fi brightness at controlled
alignment angles (0°-60°) averaged over multiple phi twist angles.
Builds a lookup table (normal, dot) → predicted_mag. ~60s.

Also: NM bypassed (same as m081), N_MAGS=40 at ±30%.

Usage:
  MICRO77_SEED=35 python3 m082_brightness_pipeline.py
"""
...
CKPT_DIR = RESULTS_DIR / f"m082_brightness_seed{TRAJ_SEED:03d}"
...
# ══════════════════════════════════════════════════════════════════════
print("=" * 60, flush=True)
print(f"m082 — Brightness-match pipeline (seed {TRAJ_SEED})")
print("=" * 60)
t_global = time.time()
...
def vectorized_phi_cost_brightness(q_anchors_xyzw, delta_qs, pab_arr,
                                    allowed_per_constraint, normals,
                                    obs_mags_arr):
    """
    Evaluate brightness-match cost for all phi values at once.
    At each constraint, predict the magnitude each allowed normal would
    produce at the computed alignment, and compare to observed magnitude.
    ...
    obs_mags_arr: (N_constraints,) observed magnitudes at constraint epochs
    ...
    for ci in range(len(delta_qs)):
        ...
        for ni in allowed:
            dots = pbs @ normals[ni]                  # (N_phi,)
            pred = predict_mag_vec(ni, dots)           # (N_phi,)
            bright_cost = (pred - obs_mag) ** 2
            best_cost = np.minimum(best_cost, bright_cost)
        costs += best_cost
    return costs
...
def eval_one_direction(wi):
    ...
    for mag in _omega_mags_s:
        ...
        for ni, qa_xyzw in _qa_anchor_sets:
            c = vectorized_phi_cost_brightness(
...
for ci, c in enumerate(candidates):
    ...
    # Evaluate brightness cost at all fine phi values
    costs = vectorized_phi_cost_brightness(
...
def refine_omega_brightness(args):
    ...
    # Final: get best phi at refined omega
    ...
    return idx, res.fun, res.x, best_phi_idx, res.nfev
