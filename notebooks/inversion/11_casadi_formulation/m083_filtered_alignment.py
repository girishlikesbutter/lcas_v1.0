#!/usr/bin/env python3
"""
m083 — Filtered alignment cost pipeline.

Same alignment cost (1-dot)^2 that worked on the original 6 seeds,
but with magnitude-filtered allowed normals at each constraint.

The co-alignment problem only affects X vs WD/ED (15° apart). At dim
peaks (mag >= 7.3, all 10 normals allowed), the alignment cost picks
+WD over +X because dot is higher, but +WD can't produce a mag 5.5 peak.

Fix: use the hi-fi calibration table to filter normals whose predicted
magnitude is >MAG_FILTER_TOL from the observed magnitude. Then apply
the original alignment cost on the filtered set.

Also: NM bypassed, N_MAGS=40 at ±30%, local omega refinement with
filtered alignment cost.

Usage:
  MICRO77_SEED=35 python3 m083_filtered_alignment.py
"""
...
CKPT_DIR = RESULTS_DIR / f"m083_filtered_seed{TRAJ_SEED:03d}"
...
# ══════════════════════════════════════════════════════════════════════
print("=" * 60, flush=True)
print(f"m083 — Filtered alignment pipeline (seed {TRAJ_SEED})")
print("=" * 60)
t_global = time.time()
...
MAG_FILTER_TOL = 1.5  # filter normals whose predicted mag is >1.5 from observed

def vectorized_phi_cost_filtered(q_anchors_xyzw, delta_qs, pab_arr,
                                  allowed_per_constraint, normals,
                                  obs_mags_arr, w):
    """
    Alignment cost (1-dot)^2 with magnitude-filtered allowed normals.

    For each constraint, compute each allowed normal's predicted magnitude.
    Filter out normals whose prediction is >MAG_FILTER_TOL from observed.
    Then apply the standard alignment cost on the filtered set.
    Falls back to unfiltered if no normal passes the filter.
    ...
    for ci in range(len(delta_qs)):
        ...
        # Compute dots for all allowed normals
        all_dots = pbs @ normals[allowed].T            # (N_phi, n_allowed)

        # Filter: for each allowed normal, check if predicted mag is within tolerance
        # Use the MAX dot across all phis as a representative alignment for filtering
        # (conservative — if the normal can produce the right mag at its best alignment,
        # keep it for all phis)
        max_dots = all_dots.max(axis=0)                # (n_allowed,)
        filtered = []
        for k, ni in enumerate(allowed):
            pred_mag = float(np.interp(max_dots[k], calib_dots[::-1], mag_table[ni, ::-1]))
            if abs(pred_mag - obs_mag) < MAG_FILTER_TOL:
                filtered.append(k)

        # Fallback: if nothing passes, use all allowed
        if not filtered:
            filtered = list(range(len(allowed)))

        # Standard alignment cost on filtered set
        bds = all_dots[:, filtered].max(axis=1)        # (N_phi,)
        costs += w * (1.0 - bds) ** 2
    return costs
...
def eval_one_direction(wi):
    ...
    for mag in _omega_mags_s:
        ...
        for ni, qa_xyzw in _qa_anchor_sets:
            c = vectorized_phi_cost_filtered(
...
for ci, c in enumerate(candidates):
    ...
    costs = vectorized_phi_cost_filtered(
