#!/usr/bin/env python3
"""
m088 — Fast Beta Pipeline (speed-optimised m077).

Identical pipeline architecture to m077 (alignment cost, NM refinement,
geometric refinement, hi-fi selection). Speed optimisations only:
  1. SLERP magnitude interpolation in grid search (10 coarse → 40 fine)
  2. Relaxed ODE tolerances for grid search (rtol=1e-6)
  3. Pre-sorted dt_constraints (avoid re-sorting per call)
  NM and geometric refinement use STRICT tolerances (unchanged from m077).

Expected speedup: ~6x on grid search. No accuracy change.

Usage:
  MICRO77_SEED=93 python3 m088_fast_beta.py
  MICRO88_WORKERS=8 MICRO77_SEED=93 python3 m088_fast_beta.py
"""
...
_WORKERS = int(os.environ.get('MICRO88_WORKERS', str(_DEFAULT_WORKERS)))

# Pipeline parameters (same as m077)
N_DIRS = 2000         # Fibonacci sphere directions
N_MAGS = 40           # fine magnitude grid points (±30% range)
N_MAGS_COARSE = 10    # coarse mags for SLERP interpolation
...
NM_WORKERS = _WORKERS
GEO_WORKERS = _WORKERS
HIFI_WORKERS = min(8, _WORKERS)

# ODE tolerances — relaxed for grid, strict for NM/geo/final
GRID_RTOL = 1e-6
GRID_ATOL = 1e-8
...
CKPT_DIR = RESULTS_DIR / f"m088_fast_seed{TRAJ_SEED:03d}"
...
# ══════════════════════════════════════════════════════════════════════
print("=" * 60, flush=True)
print(f"m088 — Fast beta pipeline (seed {TRAJ_SEED})")
...
# Pre-sort forward/backward constraint indices ONCE
...
def propagate_delta_qs(omega_vec, dt_arr, rtol=1e-10, atol=1e-12):
...
def propagate_delta_qs_fast(omega_vec):
    """Grid-search version: relaxed tolerances, pre-sorted indices."""
    q_id = np.array([1.0, 0.0, 0.0, 0.0])
...
def quat_slerp_batch(q1, q2, t):
    ...
    if np.any(close):
        ...
        result[close] /= np.linalg.norm(result[close], axis=1, keepdims=True)
    far = ~close
    if np.any(far):
...
# Pre-compute SLERP interpolation map
_coarse_idx = np.clip(np.searchsorted(omega_mags_coarse, omega_mags_fine) - 1,
                       0, N_MAGS_COARSE - 2)
_slerp_t = np.clip(
    (omega_mags_fine - omega_mags_coarse[_coarse_idx]) /
    (omega_mags_coarse[_coarse_idx + 1] - omega_mags_coarse[_coarse_idx]),
    0.0, 1.0)
...
print(f"\n--- Step 2: Grid search ({N_DIRS} dirs x {N_MAGS_COARSE} coarse -> "
      f"{N_MAGS} fine mags, {N_PHI_COARSE} phis, "
      f"{len(qa_anchor_sets)} anchor normals) ---", flush=True)
...
def eval_one_direction(wi):
    ...
    # Propagate at coarse magnitudes only
    coarse_dqs = []
    for mag in _omega_mags_coarse:
        dqs = propagate_delta_qs_fast(wd * mag)
        coarse_dqs.append(dqs)

    # SLERP-interpolate to fine magnitudes and evaluate
    for mi in range(len(_omega_mags_fine)):
        ci_lo = _ci_arr[mi]
        t_val = _st_arr[mi]
        if t_val < 1e-8:
            dqs = coarse_dqs[ci_lo]
        elif t_val > 1 - 1e-8:
            dqs = coarse_dqs[ci_lo + 1]
        else:
            dqs = quat_slerp_batch(coarse_dqs[ci_lo], coarse_dqs[ci_lo + 1], t_val)
