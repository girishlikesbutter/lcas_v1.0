#!/usr/bin/env python3
"""
m100: Batch runner for multi-phi pipeline.

Runs m100_multi_phi.py on multiple seeds sequentially.
Each seed saves results independently.

Usage:
  python3 m100_batch.py           # default 10 seeds
  python3 m100_batch.py 0 27 93   # specific seeds
"""
...
BATCH_DIR = RESULTS_DIR / "m100_multi_phi"
BATCH_DIR.mkdir(exist_ok=True)

DEFAULT_SEEDS = [0, 6, 12, 14, 24, 27, 33, 36, 74, 93]
SEEDS = [int(s) for s in sys.argv[1:]] if len(sys.argv) > 1 else DEFAULT_SEEDS

SCRIPT = str(PROJECT_ROOT / "notebooks" / "inversion" / "11_casadi_formulation" / "m100_multi_phi.py")

print("=" * 70)
print(f"m100: Multi-phi batch ({len(SEEDS)} seeds)")
print(f"  Seeds: {SEEDS}")
print("=" * 70, flush=True)
...
for seed in SEEDS:
    ...
    env = os.environ.copy()
    env['MICRO100_SEED'] = str(seed)
    env['MICRO100_CKPT_DIR'] = str(ckpt)
