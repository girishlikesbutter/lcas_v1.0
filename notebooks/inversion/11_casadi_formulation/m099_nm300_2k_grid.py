#!/usr/bin/env python3
"""
m099 — NM_TOP=300 Pipeline with 2000-dir grid (batch mode).

Based on m095e. Changes:
  1. NM_TOP=300 (from 200) — captures more seeds from deep grid ranks
  2. Batch mode: processes multiple seeds sequentially
  3. Geo tolerances loosened (ftol=1e-8) — prevents L-BFGS-B spin
  4. Per-seed checkpointing at every stage
  5. Worker counts reduced to 8 to avoid memory pressure

Usage:
  python3 m099_nm300_2kgrid.py           # default 10 seeds
  python3 m099_nm300_2kgrid.py 0 27 93   # specific seeds
"""
...
print("=" * 70, flush=True)
print(f"m099: NM_TOP={NM_TOP} Pipeline, 2000-dir grid ({len(SEEDS)} seeds)")
print(f"  Seeds: {SEEDS}")
print("=" * 70)
batch_results = []

for TRAJ_SEED in SEEDS:
    CKPT_DIR = BATCH_DIR / f"seed_{TRAJ_SEED:03d}"
    CKPT_DIR.mkdir(exist_ok=True)

    # Check for existing result
    if (CKPT_DIR / "result.json").exists():
        print(f"\n  seed {TRAJ_SEED:3d}: SKIP (result.json exists)", flush=True)
        import json as _json
        with open(str(CKPT_DIR / "result.json")) as _f:
            batch_results.append(_json.load(_f))
        continue
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"  seed {TRAJ_SEED} — starting", flush=True)
    t_global = time.time()
...
# Majority vote: for each window, rank candidates. Count how many windows each candidate wins.
votes = Counter()
...
winner = [c for c in candidates if c['omega_rank'] == best_rank][0]

# If tie, use longest window as tiebreaker
...
result_json = {
    ...
    print(f"\n  RESULT seed {TRAJ_SEED}: q0={q0_err:.1f}° w_dir={w0_err:.1f}° "
          f"w_mag={w_mag_err:+.1f}% [{status}]{twin_tag} {total_time:.0f}s", flush=True)
...
print(f"{'='*70}")
print(f"\n  {'seed':>4s} {'q0':>7s} {'w_dir':>6s} {'w_mag':>7s} {'status':>8s} {'time':>5s}")
...
for r in batch_results:
    ...
    wme = w['w_mag_err_pct']
    is_tw = q0e > 170
    st = "OK" if wde < 5 and (q0e < 5 or is_tw) else "PARTIAL" if wde < 10 and (q0e < 10 or is_tw) else "FAIL"
    tw = " +Xtw" if is_tw else ""
    print(f"  {r['traj_seed']:4d} {q0e:7.1f} {wde:6.1f} {wme:+7.2f}% {st:>8s} {r['timing']['total_s']:5.0f}s{tw}")

ok = sum(1 for r in batch_results if r['winner']['w0_err'] < 5 and (r['winner']['q0_err'] < 5 or r['winner']['q0_err'] > 170))
partial = sum(1 for r in batch_results if 5 <= r['winner']['w0_err'] < 10 or (5 <= r['winner']['q0_err'] < 10 and r['winner']['q0_err'] < 170))
fail = len(batch_results) - ok - partial
print(f"\n  OK: {ok}  PARTIAL: {partial}  FAIL: {fail}  Total: {len(batch_results)}")

save_results(str(BATCH_DIR / "batch_summary.json"), {
    'seeds': SEEDS, 'nm_top': NM_TOP, 'n_dirs': N_DIRS,
