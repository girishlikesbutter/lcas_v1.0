#!/usr/bin/env python3
"""
m090 — Noise-Robust Anchor Selection.

Identical to m077 (beta pipeline) except anchor selection uses Savitzky-Golay
smoothed magnitudes instead of raw. This eliminates noise-induced anchor flipping
when two specular peaks have similar magnitudes (e.g. seed 93: ep 27 vs ep 189).

Diagnosis (m089): the anchor epoch flips across noise realizations for seeds
93, 27, 36, 74. SG7 smoothing stabilises 7/8 seeds. For genuine ties (gap < σ),
the earliest epoch wins as a deterministic tiebreaker.
...
Usage:
  MICRO90_SEED=93 python3 m090_robust_peaks.py
"""
...
TRAJ_SEED = int(os.environ.get('MICRO90_SEED', '93'))
...
CKPT_DIR = RESULTS_DIR / f"m090_seed{TRAJ_SEED:03d}"
...
# ══════════════════════════════════════════════════════════════════════
print("=" * 60, flush=True)
print(f"m090 — Robust anchor pipeline (seed {TRAJ_SEED})")
print("=" * 60)
t_global = time.time()
...
# SG7 smoothing eliminates noise-induced anchor flipping when two specular
# peaks have similar magnitudes (e.g. seed 93: ep 27 vs 189).
# For genuine ties (gap < sigma), earliest epoch wins as tiebreaker.
...
# Tiebreaker: if top-2 are within 0.05 mag (1σ), pick earlier epoch
