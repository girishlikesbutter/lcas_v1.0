#!/usr/bin/env python3
"""
m084 — Expected-dot alignment cost.

Instead of (1 - dot)^2 which assumes perfect alignment at every peak,
use (dot - expected_dot)^2 where expected_dot is computed by inverting
the hi-fi calibration table: given the observed magnitude and a candidate
normal, what alignment angle (and therefore dot) would produce that brightness?

This naturally:
  - Makes bright peaks tight constraints (mag 5.0 → expected_dot ≈ 0.9999)
  - Makes dim peaks loose constraints (mag 8.5 → expected_dot ≈ 0.99)
  - Eliminates co-alignment degeneracy (different normals at same dot
    predict different magnitudes → different expected_dots)
  - Removes the need for hard exclusion bands or magnitude filtering

Also: NM bypassed, N_MAGS=40 at ±30%.

Usage:
  MICRO77_SEED=35 python3 m084_expected_dot.py
"""
...
CKPT_DIR = RESULTS_DIR / f"m084_expdot_seed{TRAJ_SEED:03d}"
...
# ══════════════════════════════════════════════════════════════════════
print("=" * 60, flush=True)
print(f"m084 — Expected-dot pipeline (seed {TRAJ_SEED})")
print("=" * 60)
t_global = time.time()
