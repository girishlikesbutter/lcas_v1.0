#!/usr/bin/env python3
"""
m081 — Beta pipeline with NM bypass.

Same as m077 beta but SKIPS the Nelder-Mead refinement (step 3).
Step 2b candidates go directly to geometric refinement (step 4).

Motivation: NM uses the alignment cost, which is degenerate due to
co-alignment of +X/+WD/+ED normals (15° apart, dot=0.966). NM moves
good candidates AWAY from truth toward wrong alignment-cost minima.
Seed 35 had truth at step 2b rank #2 (0.3° error) and NM destroyed
it (moved to 30.2°). Same pattern in seeds 17 and 8.

Usage:
  MICRO77_SEED=93 python3 m081_skip_nm.py
"""
...
CKPT_DIR = RESULTS_DIR / f"m081_skip_nm_seed{TRAJ_SEED:03d}"
...
# ══════════════════════════════════════════════════════════════════════
print("=" * 60, flush=True)
print(f"m081 — Skip-NM pipeline (seed {TRAJ_SEED})")
print("=" * 60)
t_global = time.time()
