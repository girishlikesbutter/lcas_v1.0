#!/usr/bin/env python3
"""
m085 — Local lo-fi omega refinement.

After the coarse grid (2000 dirs, ~2.5° spacing) finds omega to ~3°,
refine with a local fine grid: 100 directions within 5° of the best
candidate, at ~0.5° spacing. At each trial omega, sweep phi, back-
propagate to q0, compute full 500-epoch lo-fi LC, compare to observed.

The lo-fi LC uses all 500 epochs — enough signal to resolve 1° omega
changes. This closes the gap from ~3° (grid limit) to sub-degree,
which reduces q0 error from ~130° to manageable levels.

Pipeline:
  1. Calibration (hi-fi brightness table)
  2. Coarse grid (2000 dirs × 40 mags, expected-dot cost)
  3. Lo-fi peak matching (top 200 → top 20)
  4. Fine phi sweep (expected-dot cost, 720 bins)
  5. LOCAL LO-FI REFINEMENT (100 dirs × 5 mags around best candidate)
  6. Hi-fi scoring

Usage:
  MICRO77_SEED=35 python3 m085_local_lofi_refine.py
"""
...
CKPT_DIR = RESULTS_DIR / f"m085_lofi_seed{TRAJ_SEED:03d}"
...
# ══════════════════════════════════════════════════════════════════════
print("=" * 60, flush=True)
print(f"m085 — Local lo-fi refinement pipeline (seed {TRAJ_SEED})")
print("=" * 60)
t_global = time.time()
...
# ══════════════════════════════════════════════════════════════════════
# STEP 4b: Local lo-fi omega refinement
# ══════════════════════════════════════════════════════════════════════
t_step4b = time.time()

# For the top candidates, search a local fine grid of omega directions
# around the coarse grid result. At each trial omega, sweep phi, back-
# propagate to q0, compute full lo-fi LC, and score by MSE vs observed.
#
# This uses all 500 epochs — enough signal to resolve 1° omega changes.

N_LOCAL_DIRS = 100    # Fibonacci sphere directions in the local patch
N_LOCAL_MAGS = 5      # magnitude grid points around candidate
LOCAL_RADIUS_DEG = 5  # search radius in degrees
N_PHI_LOCAL = 360     # phi bins for local sweep

phi_local_xy = np.linspace(0, np.pi, N_PHI_LOCAL, endpoint=False)
phi_local_z = np.linspace(0, 2 * np.pi, 2 * N_PHI_LOCAL, endpoint=False)

# Take top 3 candidates by step 2b ranking for local refinement
n_refine = min(3, len(candidates))
print(f"\n--- Step 4b: Local lo-fi omega refinement (top {n_refine} candidates, "
      f"{N_LOCAL_DIRS} dirs × {N_LOCAL_MAGS} mags in {LOCAL_RADIUS_DEG}° patch) ---",
      flush=True)

def generate_local_dirs(center_omega, n_dirs, radius_deg):
    """Generate n_dirs directions within radius_deg of center_omega direction."""
    center_dir = center_omega / np.linalg.norm(center_omega)
    # Generate Fibonacci sphere, scale to radius, rotate to center
    fib = fibonacci_sphere(n_dirs * 4)  # oversample then filter
    # Filter to within radius
    cos_r = np.cos(np.deg2rad(radius_deg))
    dots = fib @ center_dir
    mask = dots > cos_r
    local = fib[mask][:n_dirs]
    if len(local) < n_dirs:
        local = fib[np.argsort(-dots)][:n_dirs]
    return local

def eval_local_lofi(args):
    """Evaluate one (omega_dir, omega_mag) by sweeping phi and computing lo-fi LC."""
    idx, omega_vec, anchor_ni = args
    from src.computation.shadow_engine import create_no_shadow_lit_status as _no_shadow
    from src.computation.lightcurve_generator import generate_lightcurves as _gen_lc

    phi_arr = phi_local_z if anchor_ni in Z_NORMALS else phi_local_xy

    # Propagate delta-qs to constraint times, sweep phi with expected-dot cost
    dqs = propagate_delta_qs(omega_vec, dt_constraints)
    qa_arr = np.array([anchor_q_from_phi(p, unique_normals[anchor_ni],
                       pab_j2000[anchor_idx]) for p in phi_arr])
    qa_xyzw = qa_arr[:, [1, 2, 3, 0]]

    costs = vectorized_phi_cost_expected_dot(
        qa_xyzw, dqs, pab_at_constraints,
        _constraint_allowed, unique_normals, _constraint_mags,
        CONSTRAINT_WEIGHT)
    best_phi_idx = int(np.argmin(costs))
    best_qa = qa_arr[best_phi_idx]

    # Back-propagate to t=0
    bt = np.array([0.0, anchor_time])
    qb, ob = propagate_attitude(best_qa, -omega_vec, bt, "tumbling", I_tensor)
    q0_cand = qb[-1]
    w0_cand = -ob[-1]

    # Compute full lo-fi LC
    quats, _ = propagate_attitude(q0_cand, w0_cand, _obs_times, "tumbling", _I)
    n_ep = len(quats)
    R_all = Rotation.from_quat(quats[:, [1, 2, 3, 0]]).as_matrix()
    sv = _sun[:n_ep] - _sat[:n_ep]
    sv /= np.linalg.norm(sv, axis=1, keepdims=True)
    ...
    mse = float(np.mean((pred_mags - _obs_lc) ** 2))
    return idx, mse, q0_cand, w0_cand, omega_vec

# Build local grid for each top candidate
local_args = []
local_labels = []  # track which candidate each local point belongs to
for ci in range(n_refine):
    c = candidates[ci]
    omega_center = c['omega_anchor']
    anchor_ni = c['anchor_ni']
    omega_mag = np.linalg.norm(omega_center)

    # Local directions
    local_dirs = generate_local_dirs(omega_center, N_LOCAL_DIRS, LOCAL_RADIUS_DEG)

    # Local magnitudes: ±5% around candidate magnitude
    local_mags = omega_mag * np.linspace(0.95, 1.05, N_LOCAL_MAGS)

    for ld in local_dirs:
        for lm in local_mags:
            local_args.append((len(local_args), ld * lm, anchor_ni))
            local_labels.append(ci)

print(f"  {len(local_args)} total evaluations across {n_refine} candidates")

with Pool(LOFI_WORKERS) as pool:
    local_results = pool.map(eval_local_lofi, local_args)

# Find best per candidate
for ci in range(n_refine):
    my_results = [(r[1], r[2], r[3], r[4]) for r, lab in zip(local_results, local_labels) if lab == ci]
    my_results.sort(key=lambda x: x[0])
    best_mse, best_q0, best_w0, best_omega = my_results[0]

    c = candidates[ci]
    c['q0_ref'] = best_q0
    c['w0_ref'] = best_w0
    c['q0_ref_err'] = attitude_error_deg(best_q0, true_q0)
    c['w0_ref_err'] = omega_dir_err(best_w0, true_omega0)
    c['geo_cost'] = best_mse
    c['w_mag_err'] = (np.rad2deg(np.linalg.norm(best_w0)) - true_omega_mag_dps) \
                     / true_omega_mag_dps * 100

    tag = " <--" if c['w0_ref_err'] < 5 else ""
    print(f"  Cand #{ci+1}: best_mse={best_mse:.4f} | "
          f"q0={c['q0_ref_err']:.1f}° w_dir={c['w0_ref_err']:.1f}° "
          f"w_mag={c['w_mag_err']:+.1f}%{tag}")
