#!/usr/bin/env python3
"""
m102: Batch runner for multi-phi pipeline.

Runs m102_fullmse.py on multiple seeds sequentially.
Each seed saves results independently.

Usage:
  python3 m102_batch.py           # default 10 seeds
  python3 m102_batch.py 0 27 93   # specific seeds
"""
...
BATCH_DIR = RESULTS_DIR / "m102_fullmse"
BATCH_DIR.mkdir(exist_ok=True)

DEFAULT_SEEDS = [0, 6, 12, 14, 24, 27, 33, 36, 74, 93]
SEEDS = [int(s) for s in sys.argv[1:]] if len(sys.argv) > 1 else DEFAULT_SEEDS

SCRIPT = str(PROJECT_ROOT / "notebooks" / "inversion" / "11_casadi_formulation" / "m102_fullmse.py")

print("=" * 70)
print(f"m102: Full MSE batch ({len(SEEDS)} seeds)")
print(f"  Seeds: {SEEDS}")
print("=" * 70, flush=True)
...
for seed in SEEDS:
    ...
    env = os.environ.copy()
    env['MICRO102_SEED'] = str(seed)
    env['MICRO102_CKPT_DIR'] = str(ckpt)
