#!/usr/bin/env python3
"""
m101 — Multi-phi v2: top-2 per omega + full-window MSE selection.

Fixes from m100:
  1. Send BOTH NM-selected phi (phi#0) AND best-by-geo phi to hi-fi per omega
     (m100 sent only best-by-geo, which sometimes picked a worse phi)
  2. Select winner by full-window MSE instead of multi-window majority vote
     (vote is noisy — seed 0 regressed because vote picked wrong in 2/4 windows)

Architecture: same grid/NM/multi-phi/geo as m100, different phi→hifi + selection.

Usage:
  MICRO101_SEED=14 python3 m101_multi_phi_v2.py
"""
...
TRAJ_SEED = int(os.environ.get('MICRO101_SEED', '14'))
...
# SETUP
# ══════════════════════════════════════════════════════════════════════
CKPT_DIR = Path(os.environ.get('MICRO101_CKPT_DIR',
                str(RESULTS_DIR / f"m101_multi_phi_v2" / f"seed_{TRAJ_SEED:03d}")))
CKPT_DIR.mkdir(parents=True, exist_ok=True)
_log_file = open(str(CKPT_DIR / "pipeline.log"), "w")
sys.stdout = Tee(sys.__stdout__, _log_file)

print("=" * 60, flush=True)
print(f"m101 — Multi-phi v2 (seed {TRAJ_SEED})")
...
def refine_one_geo(args):
    ...
    res = minimize(geometric_cost, x0, method='L-BFGS-B', options={'maxiter': 100, 'maxfun': 1500, 'ftol': 1e-8, 'gtol': 1e-6})
...
# Select top-2 phi per omega for hi-fi: NM-selected (phi#0) + best-by-geo if different
hifi_candidates = []
for rank in range(len(deduped)):
    omega_cands = [c for c in multi_phi_candidates if c['omega_rank'] == rank]
    # Always include NM-selected phi (phi_rank=0)
    nm_phi = [c for c in omega_cands if c['phi_rank'] == 0][0]
    nm_phi['hifi_idx'] = len(hifi_candidates)
    hifi_candidates.append(nm_phi)
    # Also include best-by-geo if it's a different phi
    best_geo = min(omega_cands, key=lambda c: c['geo_cost'])
    if best_geo['phi_rank'] != 0:
        best_geo['hifi_idx'] = len(hifi_candidates)
        hifi_candidates.append(best_geo)

print(f"\nTop-2-per-omega for hi-fi ({len(hifi_candidates)} candidates):")
...
# Selection: full-window MSE (not vote — vote is noisy, see m100 seed 0)
print(f"\nWindow results:")
...
winner = min(hifi_candidates, key=lambda c: c['hifi_windows'].get('full', 999))
print(f"\nSelected by full-window MSE: w#{winner['omega_rank']+1} phi#{winner['phi_rank']+1}")
