...
def setup_experiment(
    ...
    # Epochs
    start_et = ctx.spice_handler.utc_to_et(ctx.config.simulation_defaults.start_time)
    if end_time_utc is not None:
        end_et = ctx.spice_handler.utc_to_et(end_time_utc)
    else:
        end_et = ctx.spice_handler.utc_to_et(ctx.config.simulation_defaults.end_time)
    ctx.epochs = np.linspace(start_et, end_et, n_observations)
