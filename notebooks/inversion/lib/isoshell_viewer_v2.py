#!/usr/bin/env python3
"""
Interactive isoshell light curve viewer — HI-FI phase-invariant (v2 surrogate).

Exact port of isoshell_viewer.py with only the per-vertex flux and
per-direction target-flux computations swapped from the hand-rolled lofi
Ashikhmin-Shirley BRDF to the v2 surrogate evaluated at k1 = k2 = PAB_body
(phase-invariant, no circular assumption about body-frame k1 azimuth — see
the session discussion 2026-04-18).

Downstream contour/loop/centroid code is unchanged; the flux value it
operates on here is a pseudo-flux 10^(-mag/2.5) so log10(flux) = -mag/2.5.
Iso-contour topology is preserved; absolute scale is irrelevant.

Usage:
    python isoshell_viewer_v2.py [--seed 28] [-o output.html]
"""
...
# v2 surrogate package — loaded lazily in _get_v2()
_SURROGATE_PKG = Path.home() / "surrogate_model" / "surrogate_model"
if str(_SURROGATE_PKG) not in sys.path:
    sys.path.insert(0, str(_SURROGATE_PKG))
...
# IS-901 phase-invariant convention (same as hifi_isoshell_viewer.py):
#   panel_deg = 0, dish_deg = 15 (= original ad_angle_deg default).
# Absolute obs_dist only shifts the magnitude constant; iso-contour topology
# is scale-invariant. Pick a GEO-ish value to keep the magnitudes in the
# training-distribution range.
_PANEL_DEG = 0.0
_OBS_DIST_KM = 42000.0
...
def _get_v2():
    ...
    if _V2_MODEL is None:
        ...
        _V2_MODEL = SurrogateModel.load_default()
    return _V2_MODEL
...
def _compute_surface_flux(verts, components=None, ad_angle_deg=15.0):
    """Hi-fi phase-invariant (k1 = k2 = vert) per-vertex pseudo-flux via v2 surrogate.

    Drop-in replacement for the lofi hand-rolled BRDF. `components` is kept
    in the signature for compatibility but is ignored (v2 has its own
    internal geometry). `ad_angle_deg` is forwarded as the v2 `dish_deg`.

    Returns pseudo-flux 10^(-mag/2.5); log10(flux) = -mag/2.5, so downstream
    iso-contour extraction continues to work unchanged.
    """
    model = _get_v2()
    verts = np.asarray(verts, dtype=np.float64)
    N = len(verts)
    dist = np.full(N, _OBS_DIST_KM, dtype=np.float64)
    mag = np.asarray(model.predict_magnitude(verts, verts, _PANEL_DEG,
                                              float(ad_angle_deg), dist),
                     dtype=np.float64)
    return np.power(10.0, -mag / 2.5)
...
def _compute_target_flux(pab_dir, components=None, ad_angle_deg=15.0):
    """Hi-fi phase-invariant pseudo-flux at a single PAB direction via v2."""
    model = _get_v2()
    pab = np.asarray(pab_dir, dtype=np.float64).reshape(1, 3)
    dist = np.array([_OBS_DIST_KM], dtype=np.float64)
    mag = float(np.asarray(model.predict_magnitude(
        pab, pab, _PANEL_DEG, float(ad_angle_deg), dist))[0])
    return float(10.0 ** (-mag / 2.5))
