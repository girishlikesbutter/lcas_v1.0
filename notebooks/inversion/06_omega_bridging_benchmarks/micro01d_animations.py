"""
micro01d: 3D Plotly animations for quaternion nudge experiment.

Generates shadow-pattern animations (yellow=lit, dark blue=shadowed,
purple=back-culled) for every trajectory × nudge combination from micro01c.
This verifies visually whether self-shadowing explains why lo-fi fails for
trajectories 2 & 3.

Output: data/results/inversion_diagnostics/micro01d_animations/
...
OUT = Path("data/results/inversion_diagnostics/micro01d_animations")
...
# ── Pre-generate random nudge axes (deterministic, same as micro01c) ─
...
for ti, (q0, label) in enumerate(zip(Q0S, LABELS)):
    ...
    # Observed LC (hi-fi + noise, same seed as micro01c)
    ...
    index_rows.append((f"traj{ti}_{label}/true_hifi.html",
                       f"micro01c_lofi_qnudge_{om_tag}_traj{ti}_nudge0.png",
                       label, "true", "hifi", "-"))
    ...
    for nudge_deg in NUDGE_DEGS:
        ...
        for di in range(n_dirs):
            ...
            micro01c_plot = f"micro01c_lofi_qnudge_{om_tag}_traj{ti}_nudge{nudge_deg}.png"
            index_rows.append((f"traj{ti}_{label}/{fname}", micro01c_plot,
                               label, str(nudge_deg), "lofi",
                               str(di) if nudge_deg > 0 else "-"))

# ── Write index.md ───────────────────────────────────────────────────
with open(OUT / "index.md", "w") as f:
    f.write("# micro01d — Animation Index\n\n")
    f.write("| Animation | micro01c plot | Trajectory | Nudge° | Fidelity | Dir |\n")
    f.write("|-----------|-------------|------------|--------|----------|-----|\n")
