"""
micro01b: Peak omega recovery with quaternion nudge errors.

Same as micro01 but simplified: only use (k-1, k) pair, only propagate
from q_k. Before propagating, nudge q_k off by 1–5 degrees along
5 random directions per magnitude to capture anisotropic sensitivity.
"""
...
for ti, (q0, label) in enumerate(zip(Q0S, LABELS)):
    ...
    for nudge_deg in NUDGE_DEGS:
        ...
        ax.set_title(f'Traj {ti} ({label}) — nudge {nudge_deg}° x{n_dirs}dirs [σ={ctx.noise_sigma}]')
        ax.legend(fontsize=7, ncol=2); ax.invert_yaxis()
        fig.savefig(OUT / f"micro01b_qnudge_{om_tag}_traj{ti}_nudge{nudge_deg}.png",
